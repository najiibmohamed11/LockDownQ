          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-16 gap-2 mb-4 border-b pb-2">
              ...
              {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} className="col-span-1 w-full h-6 bg-purple-50 rounded"></div>
              ))}
            </div>
  
            {/* Table Rows */}
            {Array.from({ length: 4 }).map((_, rowIndex) => (
              <div key={rowIndex} className="grid grid-cols-16 gap-2 py-4 border-b">
                <div className="col-span-3 w-full h-6 bg-gray-100 rounded"></div>
                <div className="col-span-1 w-full h-6 bg-blue-100 rounded"></div>
                {Array.from({ length: 12 }).map((_, i) => (
                  <div key={i} className="col-span-1 w-8 h-8 mx-auto rounded-full bg-gray-100"></div>
                ))}
              </div>
            ))}
          </div>
